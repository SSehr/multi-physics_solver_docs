...
Various material properties can be defined as functions instead of constant values. These functions can then be used to modify the property over the course of the simulation. The functions are set within the geometry definition file, either as separate functions or as lambda functions. The property functions are then assigned to a material domain through set($<$function\+\_\+name$>$). These functions are then called when a mesh is created and the properties are constant will that mesh is in use. I.\+e. the properties can only change once per diffusion time step.\hypertarget{index_autotoc_md30}{}\doxysubsection{Example functions}\label{index_autotoc_md30}
Example lamda functions for temperature and initial strain can be found in \mbox{\hyperlink{helper__macros_8hpp_source}{helper\+\_\+macros.\+hpp}}. See the geometry definition files for example uses. 